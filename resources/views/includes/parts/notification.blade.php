...
@if($notification->data['status_projeto'] == 'Indeferido')
    <a class="dropdown-item"  href="{{route('corrigir-projeto-user', [$notification->data['id'], $notification->id]) }}">{{$notification->data['titulo_projeto']}} - <b class="text-danger">{{$notification->data['status_projeto']}}</b></a>
@endif    
