...
class Documento extends Model
{
    ...
    public function salvar(int $id, Array $dados): Array
    {
        ...
        $dataTable = $projetoController->stringToArray($dados['table-documento']);
        for ($i = 0; $i < count($dataTable); $i++) {
            $insert = $this->create([
                                    'desc_documento' => $dataTable[$i],
                                    'projeto_id'    => $id
                                ]);
        if ($insert) {
                $cont ++;
        ...
        return [
                'success' => false,
                'message' => 'Erro ao tentar inserir dados do Documento:'
            ];    
    }
    ...
    public function atualizar(Array $request): Array
    {
        if ($this->where('projeto_id', $request['id'])->first() == null) {
            ...
            if ($delete) {
                ...
                return [
                    'success' => false,
                    'message' => 'erro ao tentar excluir dados de documentos'
                ];
            }
        }
    }
